...
namespace Microsoft.Samples.Kinect.KinectFusionExplorer
{
    ...
    public partial class MainWindow : Window, INotifyPropertyChanged, IDisposable
    {
        ...
        //CSe desborda el contador en 1 seg.
        private DispatcherTimer TimerSeg;

        //Contar 30 seg.
        private DispatcherTimer Timer30Seg;

        //Contador de seg
        private int TenSec;

        //5 seg para q se reinicie la reconstruccion y empiece la modelacion,tiempo para q el usuario se acomode
        private int tiempodePreparacion = 5;

        //45 q durara la modelacion y al terminar este tiempo se procedera a salvar la nube final
        private int tiempodeModelacion = 5;

        private BodyFrameReader bodyFRameReader = null;
        private BodyIndexFrameReader bodyIndexReader = null;
        private FrameDescription bodyIndexDescription = null;
        private JointsCoo myJoints ;
        ...
        public MainWindow()
        {
            ...
            myJoints = new JointsCoo();
            TenSec = tiempodePreparacion;
            trackCount = 0;
            trackComplete = false;
        ...
        private void WindowLoaded(object sender, RoutedEventArgs e)
        {
            ...
            //Preparar el Body index separado de los demas frames
            bodyIndexReader = this.sensor.BodyIndexFrameSource.OpenReader();
            this.bodyIndexReader.FrameArrived += bodyIndexReader_FrameArrived;
            this.bodyIndexDescription = this.sensor.BodyIndexFrameSource.FrameDescription;

            //Preparar el Body (Skeletons)
            bodyFRameReader = this.sensor.BodyFrameSource.OpenReader();
        ...
        private void BodyFRameReader_FrameArrived(object sender, BodyFrameArrivedEventArgs e)
        {
            ...
            if (dataOk)
            {
                ...
                {
                    //Asegurarme de que esten la mayoria de las mediciones listas en el momento de q la persona este lista 
                    if (body.IsTracked && !trackComplete && StartTraking)
                    {
                        ...
                        for (int i = 0; i < joints.Count; i++)
                        {
                            ...
                            {
                                //myJoints.HeadCoo.Add(joints[JointType.Head].Position);
                                myJoints.HeadCoo = joints[JointType.Head].Position;
                                trackCount++;
                            }

                            if (joints[JointType.Neck].TrackingState == TrackingState.Tracked)
                            {
                               // myJoints.NeckCoo.Add(joints[JointType.Neck].Position);
                                myJoints.NeckCoo = joints[JointType.Neck].Position;
                                trackCount++;
                            }

                            if (joints[JointType.SpineShoulder].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.SpineShoulderCoo.Add(joints[JointType.SpineShoulder].Position);
                                myJoints.SpineShoulderCoo = joints[JointType.SpineShoulder].Position;
                                trackCount++;
                            }

                            if (joints[JointType.SpineMid].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.SpineMidCoo.Add( joints[JointType.SpineMid].Position);
                                myJoints.SpineMidCoo = joints[JointType.SpineMid].Position;
                                trackCount++;
                            }

                            if (joints[JointType.SpineBase].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.SpineBaseCoo.Add( joints[JointType.SpineBase].Position);
                                myJoints.SpineBaseCoo = joints[JointType.SpineBase].Position;
                                trackCount++;
                            }

                            if (joints[JointType.ShoulderLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.ShoulderLeftCoo.Add(joints[JointType.ShoulderLeft].Position);
                                myJoints.ShoulderLeftCoo = joints[JointType.ShoulderLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.ElbowLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.ElbowLeftCoo.Add( joints[JointType.ElbowLeft].Position);
                                myJoints.ElbowLeftCoo = joints[JointType.ElbowLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.WristLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.WristLeftCoo.Add(joints[JointType.WristLeft].Position);
                                myJoints.WristLeftCoo = joints[JointType.WristLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.HandLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.HandLeftCoo.Add(joints[JointType.HandLeft].Position);
                                myJoints.HandLeftCoo = joints[JointType.HandLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.HandTipLeft].TrackingState == TrackingState.Tracked)
                            {
                                // myJoints.HandTipLeftCoo.Add( joints[JointType.HandTipLeft].Position);
                                myJoints.HandTipLeftCoo = joints[JointType.HandTipLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.ThumbLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.ThumbLeftCoo.Add(joints[JointType.ThumbLeft].Position);
                                myJoints.ThumbLeftCoo = joints[JointType.ThumbLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.ShoulderRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.ShoulderRightCoo.Add( joints[JointType.ShoulderRight].Position);
                                myJoints.ShoulderRightCoo = joints[JointType.ShoulderRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.ElbowRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.ElbowRightCoo.Add( joints[JointType.ElbowRight].Position);
                                myJoints.ElbowRightCoo = joints[JointType.ElbowRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.WristRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.WristRightCoo.Add( joints[JointType.WristRight].Position);
                                myJoints.WristRightCoo = joints[JointType.WristRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.HandRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.HandRightCoo.Add( joints[JointType.HandRight].Position);
                                myJoints.HandRightCoo = joints[JointType.HandRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.HandTipRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.HandTipRightCoo.Add(joints[JointType.HandTipRight].Position);
                                myJoints.HandTipRightCoo = joints[JointType.HandTipRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.ThumbRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.ThumbRightCoo.Add(joints[JointType.ThumbRight].Position);
                                myJoints.ThumbRightCoo = joints[JointType.ThumbRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.HipLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.HipLeftCoo.Add( joints[JointType.HipLeft].Position);
                                myJoints.HipLeftCoo = joints[JointType.HipLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.KneeLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.KneeLeftCoo.Add( joints[JointType.KneeLeft].Position);
                                myJoints.KneeLeftCoo = joints[JointType.KneeLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.AnkleLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.AnkleLeftCoo.Add(joints[JointType.AnkleLeft].Position);
                                myJoints.AnkleLeftCoo = joints[JointType.AnkleLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.FootLeft].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.FootLeftCoo.Add( joints[JointType.FootLeft].Position);
                                myJoints.FootLeftCoo = joints[JointType.FootLeft].Position;
                                trackCount++;
                            }

                            if (joints[JointType.HipRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.HipRightCoo.Add(joints[JointType.HipRight].Position);
                                myJoints.HipRightCoo = joints[JointType.HipRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.KneeRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.KneeRightCoo.Add(joints[JointType.KneeRight].Position);
                                myJoints.KneeRightCoo = joints[JointType.KneeRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.AnkleRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.AnkleRightCoo.Add(joints[JointType.AnkleRight].Position);
                                myJoints.AnkleRightCoo = joints[JointType.AnkleRight].Position;
                                trackCount++;
                            }

                            if (joints[JointType.FootRight].TrackingState == TrackingState.Tracked)
                            {
                                //myJoints.FootRightCoo.Add(joints[JointType.FootRight].Position);
                                myJoints.FootRightCoo = joints[JointType.FootRight].Position;
                                trackCount++;
                            }
        ...
        private void WindowClosing(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            if (this.bodyFRameReader!=null)
            {
                ...
                this.bodyIndexReader.Dispose();
                this.bodyIndexReader = null;
            }

            if (this.bodyFRameReader!=null)
            {
                bodyFRameReader.FrameArrived -= this.BodyFRameReader_FrameArrived;
            ...
            try
            {
                ...
                if (multiSourceFrame != null)
                {
                    ...
                    lock (this.rawDataLock)
                    {
                        ...
                        //BodyIndexFrameReference bodyIndexReference = multiSourceFrame.BodyIndexFrameReference;

                        colorFrame = colorFrameReference.AcquireFrame();
                        depthFrame = depthFrameReference.AcquireFrame();
                        //BodyIndexFrame bodyIndexFrame = multiSourceFrame.BodyIndexFrameReference.AcquireFrame();
        ...
        void bodyIndexReader_FrameArrived(object sender, BodyIndexFrameArrivedEventArgs e)
        {
            this.bodyIndexData = new byte[depthWidth * depthHeight];
            ...
            using (BodyIndexFrame bodyIndexFrame = e.FrameReference.AcquireFrame())
            {
                if (bodyIndexFrame != null)
                {
                    ...
                    //for (int i = 0; i < depthImagePixels.Length; i++)
                    //{
                    //    // si bodyIndexData[i]==255 es true entonces ese pixel no corresponde a una persona
                    //    if (bodyIndexData[i] == 0xff)
                    //    {
                    //        depthImagePixels[i] = 10000;
                    //    }
                    //}
        ...
        /// <summary>
        /// Handler for click event from "Escanear" button
        /// </summary>
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event arguments</param>  
        private void ResetCameraButtonClick(object sender, RoutedEventArgs e)
        {
            TimerSeg = new DispatcherTimer();
            TimerSeg.Tick += TimerSeg_Tick;
            TimerSeg.Interval = new TimeSpan(0, 0, 1);      //Contar un segundo
            TimerSeg.Start();

            TimerText.Text = TenSec.ToString();
            var path= Directory.GetCurrentDirectory() + "\\HumanVolumeCalculator.exe";
        ...
        private void TimerSeg_Tick(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //Geting ready the timer for 30 seconds to save the mesh
                Timer30Seg = new System.Windows.Threading.DispatcherTimer();
                Timer30Seg.Tick += Timer30Seg_Tick;
                Timer30Seg.Interval = new TimeSpan(0, 0, 1);
                TenSec = tiempodeModelacion;
                //LabelClock.Content = TenSec.ToString();
                Timer30Seg.Start();

            }
        }

        private void Timer30Seg_Tick(object sender, EventArgs e)
        {
            if (TenSec != 1)
            {
                TenSec--;
            ...
            else
            {
                ...
                Timer30Seg.Stop();
                TenSec = tiempodeModelacion;
                this.sensor.Close();                  //Detener la obtencion de datos desde el sensor
                                                      //saving the mesh
                ...
                try
                {
                    ...
                    if (true == dialog.ShowDialog())
                    {
                        ...
                        using (StreamWriter writer = new StreamWriter(dialog.FileName))
                        {
                            ...
                            string[] path = dialog.FileName.Split(delimeter, 2);
                            string textPath = path[0] + "JointsCoo.txt";

                            WriteJointsCoordinate(textPath);

                            System.Diagnostics.Process.Start(Directory.GetCurrentDirectory() + "HumanVolumeCalculator.exe");
                            Application.Current.Shutdown();
        ...
        /// <param name="sender">Event sender</param>
        /// <param name="e">Event arguments</param>
        private void WriteJointsCoordinate(string path)
        {
            using (StreamWriter writerJoint = new StreamWriter(path))
            {
                writerJoint.WriteLine("Head");
                string hipFile =(myJoints.HeadCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HeadCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HeadCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("Neck");
                hipFile = (myJoints.NeckCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.NeckCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.NeckCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("ShoulderCenter");
                hipFile =(myJoints.SpineShoulderCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.SpineShoulderCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.SpineShoulderCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("ShoulderLeft");
                hipFile = (myJoints.ShoulderLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ShoulderLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ShoulderLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("ElbowLeft");
                hipFile = (myJoints.ElbowLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ElbowLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ElbowLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("WristLeft");
                hipFile = (myJoints.WristLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.WristLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.WristLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("HandLeft");
                hipFile = (myJoints.HandLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);
                
                writerJoint.WriteLine("ShoulderRight");
                hipFile = (myJoints.ShoulderRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ShoulderRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ShoulderRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("ElbowRight");
                hipFile = (myJoints.ElbowRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ElbowRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ElbowRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("WristRight");
                hipFile = (myJoints.WristRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.WristRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.WristRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("HandRight");
                hipFile = (myJoints.HandRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("SpineMid");
                hipFile = (myJoints.SpineMidCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.SpineMidCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.SpineMidCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("SpineBase");
                hipFile = (myJoints.SpineBaseCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.SpineBaseCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.SpineBaseCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("HipLeft");
                hipFile = (myJoints.HipLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HipLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HipLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("KneeLeft");
                hipFile = (myJoints.KneeLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.KneeLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.KneeLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("AnkleLeft");
                hipFile = (myJoints.AnkleLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.AnkleLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.AnkleLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("FootLeft");
                hipFile = (myJoints.FootLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.FootLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.FootLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("HipRight");
                hipFile = (myJoints.HipRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HipRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HipRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("KneeRight");
                hipFile = (myJoints.KneeRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.KneeRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.KneeRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("AnkleRight");
                hipFile = (myJoints.AnkleRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.AnkleRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.AnkleRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("FootRight");
                hipFile = (myJoints.FootRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.FootRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.FootRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("HandLeftTip");
                hipFile = (myJoints.HandTipLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandTipLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandTipLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("ThumbLeft");
                hipFile = (myJoints.ThumbLeftCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ThumbLeftCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ThumbLeftCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("HandRigthTip");
                hipFile = (myJoints.HandTipRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandTipRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.HandTipRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);

                writerJoint.WriteLine("ThumbRight");
                hipFile = (myJoints.ThumbRightCoo).X.ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ThumbRightCoo).Y).ToString(CultureInfo.InvariantCulture) + " ";
                hipFile += (-(myJoints.ThumbRightCoo).Z).ToString(CultureInfo.InvariantCulture);
                writerJoint.WriteLine(hipFile);
        ...
        private Point3D Promedio(List<CameraSpacePoint> list)
        {
            Point3D pProm = new Point3D();
